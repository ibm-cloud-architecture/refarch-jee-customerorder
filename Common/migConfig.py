...
userId=input("Enter your Database's UserName ")
password=input("Enter your Database's Password ")
db2jcc4=input("Enter the path for db2jcc4 jar ")
db2jcc_license_cisuz=input("Enter the path for db2jcc_license_cisuz jar ")
db2jcc_license_cu=input("Enter the path for db2jcc_license_cu ")
...
AdminConfig.create('JAASAuthData', GlobalSecurityVar, [['password', password], ['userId', userId], ['alias', 'localhostNode01/DBUser'], ['description', '']])

print 'Starting Creating Connection Factories'

print 'Starting Creating JDBC Providers'
AdminConfigVar_11=AdminConfig.create('JDBCProvider', Cell, [['providerType', 'DB2 Using IBM JCC Driver (XA)'], ['name', 'DB2_Using_IBM_JCC_Driver_(XA)'], ['description', 'Two-phase commit DB2 JCC provider that supports JDBC 4.0 using the IBM Data Server Driver for JDBC and SQLJ. IBM Data Server Driver is the next generation of the DB2 Universal JCC driver. Data sources created under this provider support the use of XA to perform 2-phase commit processing. Use of JDBC driver type 2 on WebSphere Application Server for Z/OS is not supported for data sources created under this provider. This provider is configurable in version 7.0 and later nodes.'], ['implementationClassName', 'com.ibm.db2.jcc.DB2XADataSource'], ['classpath', db2jcc4 + ';' + db2jcc_license_cisuz + ';' + db2jcc_license_cu], ['xa', 'true']])
